...
import { buildWeekendAcca } from '@/lib/trends/acca-builder';
...
export async function POST(req: Request) {
  ...
  const acca = await buildWeekendAcca(from, to, supabase);

  if (!acca) {
    return NextResponse.json({ message: 'Not enough qualifying tips for weekend acca' });
  }
  ...
  if (error) {
    console.error('Failed to insert weekend acca:', error);
